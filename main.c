...
int main()
{
	...
	saveAirline(&company, airlineFileName, compress);
...
int initManagerAndAirline(AirportManager* pManager, Airline* pCompany,const char* ailineFileName , int compress)
{
	...
	{
		if(compress == 1)
			return initAirlineFromFile(pCompany, pManager, ailineFileName, readAirlineDataFromBinaryFileCompressed);
		return initAirlineFromFile(pCompany, pManager, ailineFileName, loadAirlineFromFile);
	}
	else
...
int saveAirline(const Airline* pComp, const char* fileName, int compress)
{
	if (compress == 1)
		return saveAirlineToFileCompressed(pComp, fileName);
	return saveAirlineToFile(pComp, fileName);
}
