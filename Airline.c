...
int		initAirlineFromFile(Airline* pComp, AirportManager* pManaer, const char* fileName,
	int(*readFromFile)(Airline*, const AirportManager*, const char*))
{
	L_init(&pComp->flighDateList);
	if (readFromFile(pComp, pManaer, fileName))
...
int		saveAirlineToFileCompressed(const Airline* pComp, const char* fileName)
{
	...
	data[1] = ((pComp->flightCount & 0x1) << 7) | (pComp->sortOpt << 4) | len;
...
int		readAirlineDataFromBinaryFileCompressed(Airline* pComp, const AirportManager* pManager,
	const char* fileName)
{
	int i;
	...
	pComp->flightCount = (data[0] << 1) | (data[1] >> 7);
	...
	int len = (data[1] & 0xF);
	pComp->name = (char*)calloc(len + 1, sizeof(char));
	...
	for (i = 0; i < pComp->flightCount; i++)
	{
		...
		if (!loadFlightFromFileCompressed(pComp->flightArr[i], fp))
		{
			free(pComp->flightArr);
			fclose(fp);
			return 0;
